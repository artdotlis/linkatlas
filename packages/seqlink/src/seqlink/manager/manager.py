from cafi.constants.versions import CURRENT_VER
from mpyflow.library.sync_io.queue import SyncQueue
from mpyflow.library.work.pass_through import PassThrough
from mpyflow.library.workable.element import Workable
from mpyflow.library.worker import Worker
from mpyflow.run_wrapper import start_worker

import pickle
from pathlib import Path
from seqlink.model.ena_seq import ENAReader
from seqlink.model.extract_work import ExtractWork
from utilslink.container.bio_ent import AddDes, AddIdTaxa
from utilslink.container.conf import AgentConf
from utilslink.report.wish_list import create_ccno_wish_list
from seqlink.model.container.database import (
    ReportSeq,
    DefCmd,
    SeqStatus,
    TW_REQ_REP_E,
    TW_RES_REP_E,
    TW_REQ_UP_T,
    TW_REQ_UP_E,
)
from seqlink.model.container.sequence import SequenceResults, SeqUpdatePackage
from seqlink.model.database.proc.main_sqlite import init_seq_sqlite_database
from seqlink.model.database_worker import SEQ_DB_NAME, seq_worker_rep, seq_worker_upd

from typing import final, Self, Any
from utilslink.context.process import get_worker_ctx
from utilslink.database.sqlite_manager import DatabaseWork
from utilslink.error.exceptions import DatabaseEx
from utilslink.schema.taxa import PIDType

type _CON_ANA_E = tuple[int, str, PIDType | None, str]
type _CON_ANA = tuple[_CON_ANA_E, ...]

type _SEQ_UP_T = tuple[SeqUpdatePackage, ...]
type _CON_UP_T = tuple[AddDes | AddIdTaxa, ...]


@final
class SeqUpdateManager:
    __slots__ = ("__acf", "__work_dir", "__worker")
    __instance: Self | None = None

    def __init__(self, work_dir: Path, worker: int, agent: AgentConf, /) -> None:
        self.__work_dir = work_dir
        self.__worker = worker if worker > 1 else 1
        self.__acf = agent
        super().__init__()

    def __new__(cls, *_args: Any) -> Self:
        if cls.__instance is not None:
            return cls.__instance
        cls.__instance = super().__new__(cls)
        return cls.__instance

    def update_database(self) -> None:
        ctx = get_worker_ctx()
        seq_con_ana = SyncQueue[_CON_ANA](ctx, 4, pickle.dumps, pickle.loads)
        seq_up = SyncQueue[TW_REQ_UP_T](ctx, 8, pickle.dumps, pickle.loads)
        con_up = SyncQueue[TW_REQ_UP_T](
            ctx, self.__worker * 4, pickle.dumps, pickle.loads
        )
        database = DatabaseWork[TW_REQ_UP_E, _CON_ANA_E](
            self.__work_dir.joinpath(SEQ_DB_NAME),
            init_seq_sqlite_database,
            seq_worker_upd,
            ctx.RLock(),
        )
        ena_in = ENAReader(self.__work_dir, CURRENT_VER, self.__acf)
        extractor = ExtractWork(self.__work_dir, CURRENT_VER)

        ena_w = Workable[_SEQ_UP_T, _SEQ_UP_T](ctx, PassThrough(), ena_in)
        db_seq_w = Workable[TW_REQ_UP_T, _CON_ANA](ctx, database, seq_up)
        db_con_w = Workable[TW_REQ_UP_T, _CON_ANA](ctx, database, con_up)
        extract_w = Workable[_CON_ANA, _CON_UP_T](ctx, extractor, seq_con_ana)

        ena_worker = Worker[_SEQ_UP_T, TW_REQ_UP_T](
            "ena", (ena_w,), (db_seq_w,), False, 100
        )
        extract_info = Worker[_CON_ANA, TW_REQ_UP_T](
            "extract", (extract_w,), (db_con_w,), False, 100
        )
        db_worker_seq = Worker[TW_REQ_UP_T, _CON_ANA](
            "database_seq", (db_seq_w,), (extract_w,), False, 100
        )
        db_worker_con = Worker[TW_REQ_UP_T, _CON_ANA](
            "database_con", (db_con_w,), tuple(), False, 100
        )

        start_worker(
            self.__work_dir,
            "update sequence",
            ctx,
            (
                (1, ena_worker),
                (self.__worker, extract_info),
                (2, db_worker_seq),
                (2, db_worker_con),
            ),
            (ena_w, db_seq_w, db_con_w, extract_w),
        )


@final
class SeqReportManager:
    __slots__ = ("__database", "__work_dir")
    __instance: Self | None = None

    def __init__(
        self,
        work_dir: Path,
        /,
    ) -> None:
        self.__work_dir = work_dir
        ctx = get_worker_ctx()
        self.__database = DatabaseWork[TW_REQ_REP_E, TW_RES_REP_E](
            self.__work_dir.joinpath(SEQ_DB_NAME),
            init_seq_sqlite_database,
            seq_worker_rep,
            ctx.RLock(),
            True,
        )
        if self.status()[0] == 0:
            raise DatabaseEx("sequence database is empty")
        super().__init__()

    def __new__(cls, *_args: Any) -> Self:
        if cls.__instance is not None:
            return cls.__instance
        cls.__instance = super().__new__(cls)
        return cls.__instance

    def report(self, include_list: Path | None, /) -> None:
        output = self.__work_dir.joinpath("seqlink_report.csv")
        print(f"[REPORTING] writing to file {output!s}")
        req = (
            ReportSeq(
                include_des=create_ccno_wish_list(include_list),
            ),
        )
        with output.open("w") as out:
            for rep_con in self.__database.simple_get(req):
                for rep in rep_con:
                    if isinstance(rep, SequenceResults):
                        out.write(f"{rep.to_string()}\n")

    def status(self) -> tuple[int, float]:
        for res_con in self.__database.simple_get((DefCmd.sta,)):
            for res in res_con:
                if isinstance(res, SeqStatus):
                    return res.cnt, res.last_update
        return 0, 0.0

    def close(self) -> None:
        self.__database.on_close()
